...
from todo_manager.gui.theme import (
    ...
    RADIUS_MD, RADIUS_SM, RADIUS_XL, qcolor,
)
...
class TaskDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self._parent_info_label.setFont(QFont(FONT_FAMILY, FONT_SIZE_XS))
        ...
        self._error_label.setFont(QFont(FONT_FAMILY, FONT_SIZE_XS))
    ...
    def _build_delete_body(self):
        ...
        self._delete_text.setFont(QFont(FONT_FAMILY, FONT_SIZE_SM))
        self._delete_text.setWordWrap(True)
        self._body_layout.addWidget(self._delete_text)

        self._delete_sub_hint = QLabel()
        self._delete_sub_hint.setFont(QFont(FONT_FAMILY, FONT_SIZE_XS))
    ...
    def set_task_data(self, task):
        """预填编辑数据。"""
        if self._mode == self.MODE_DELETE:
            sub_count = len([s for s in (task.subtasks or []) if not s.deleted])
