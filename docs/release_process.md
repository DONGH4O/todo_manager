...
> 版本：v0.1  
> 日期：2026-05-24  
> 覆盖里程碑：M6 双平台打包、M6.5 React 桌面正式发布界面集成、M6.6 应用图标与桌面品牌资产、M7 CI release dry-run、M7.5 桌面 GUI 轻量化优化
...
- M7.5 起发布包不应回流 QtWebEngine debug/devtools、多余 QtWebEngine locale 或未使用的 Qt add-on 模块。
...
- QtWebEngine debug/devtools 资源。
- `zh-CN` / `en-US` 之外的 QtWebEngine locale pack。
- Qt3D、Charts、DataVisualization、Multimedia、Pdf、Quick3D 等正式 React 桌面壳未使用的 Qt 模块。
...
Windows/macOS 完整发布包构建后，建议复跑体积审计：

```powershell
.\.venv\Scripts\python.exe scripts\build.py audit-size
```

也可以显式指定 release 目录和 zip：

```powershell
.\.venv\Scripts\python.exe scripts\audit_release_size.py --release-dir dist\TodoManager --zip dist\TodoManager-windows-YYYY-MM-DD.zip
```

如需在 PyInstaller workpath 尚未清理时审计 archive 内部分类，可增加：

```powershell
.\.venv\Scripts\python.exe scripts\audit_release_size.py --release-dir dist\TodoManager --zip dist\TodoManager-windows-YYYY-MM-DD.zip --pyinstaller-workpath build\windows-gui
```

当前 Windows M7.5 验证基线见 `docs/m7_5_validation_report.md`：

- `todo-gui.exe`：218,866,808 bytes -> 170,487,859 bytes。
- Windows zip：227,082,390 bytes -> 178,811,133 bytes。
- `desktop-react/`：约 0.82 MiB，确认 React 静态资源不是主要体积来源。

若轻量化与稳定性冲突，以稳定性优先；先恢复 QtWebEngine 渲染、中文字体、日期弹层和 bridge，再缩小裁剪范围。
